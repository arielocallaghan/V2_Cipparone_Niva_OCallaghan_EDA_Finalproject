...
\newpage
### List of Tables
- Table \@ref(tab:Coal-Summary-Table): Coal Summary Table
- Table \@ref(tab:Health-Outcomes-Table): Health Outcomes Table
- Table \@ref(tab:Health-Z-Score-Summary-Table): Community Health Z Score Summary Table
- Table \@ref(tab:ACS-Summary-Table): Demographic Variables Summary Table

\newpage
\listoffigures 
### List of Figures and Maps (in order of appearance)
- Figure \@ref(fig:Coal-prod-over-time): Coal production over time across PA counties
- Map A: Map of Health Outcome Z-Scores by County in 2019
- Map B: Map of Health Outcome Ranks by County in 2019
- Map C: Map of Health Outcome Z-Scores by County in 2010
- Map D: Map of Health Outcome Ranks by County in 2010
- Figure \@ref(fig:dem-counties-graph): County values for socioeconomic variables in 2019
- Figure \@ref(fig:time-z-all-counties): Health factors and outcomes over time across counties
- Figure \@ref(fig:sum-coal-over-time): Total coal production across all counties over time
- Figure \@ref(fig:schuylkill-health-factors): Health factor scores over time in Schuylkill county
- Figure \@ref(fig:schuylkill-health-outcomes): Health outcome scores over time in Schuylkill county
- Figure \@ref(fig:armstrong-health-factors): Health factor scores over time in Armstrong county
- Figure \@ref(fig:armstrong-health-outcomes): Health outcome scores over time in Armstrong county
- Figure \@ref(fig:clearfield-health-factors): Health factor scores over time in Clearfield county
- Figure \@ref(fig:clearfield-health-outcomes): Health outcome scores over time in Clearfield county
- Figure \@ref(fig:Mixed-correlation-plot): Mixed correlation plot for all model variables
- Figure \@ref(fig:mlr-assumption-plots): Plot of multiple linear regression assumptions
- Figure \@ref(fig:mlr-scatter): Linear model scatterplot relationships between explanatory and response variables
- Figure \@ref(fig:sulfate-plot): Sulfate levels in Pennsylvania mine drainage
...
#Kable library additions for health outcomes
library(kableExtra)
library(glue)
library(knitr)
...
We examine this question through the use of four distinct county-level datasets: coal production data from the U.S. Energy Informational Administration (USIEA), census data from the American Community Survey (ACS), county health rankings data from the University of Wisconsin’s Community Health Rankings and Roadmap program, and Water quality data from the Water Quality Portal, a partnership between United States Geological Survey and the Environmental Protection Agency. These datasets collectively covered a broad enough time series to observe impacts in coal production over time. All these datasets focus on the state of Pennsylvania.
...
```{r Coal Wrangling Code, echo=TRUE}
#initial uploading of all the coal data for each distinct year -- this could not be expedited with data stripping because there were separate links to downloaded spreadsheets instead of links to another website
...
**Coal Summary Table**
The summary table below provides descriptive statistics for each of the parameters included.
```{r Coal-Summary-Table, echo=TRUE, fig.cap="Summary Statistics of coal production in Pennsylvania"}

#We can make the summary table of the data within the dataset

coal.data.summary <- aggCoalSlim2 %>% 
    rename("Coal Production per Year (tons)" = coal.prod.by.year, "Weighted Year of Peak Production" = year.peak.prod, "Total Coal Produced" = sum.coal.prod) %>% 
    psych::describe(na.rm=TRUE) %>%
    as_tibble(rownames="rowname") %>% 
  filter(rowname!="County*" & rowname!="Year")

coal.table<-coal.data.summary %>%
...
Table \@ref(tab:Coal-Summary-Table): Summary statistics of coal production in Pennsylvania
...
```{r Data Wrangling for Water Quality Data, echo=TRUE}
...
```{r Spatial Data Code Wrangling, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r Health-Outcomes-Table, echo=TRUE, fig.cap="Descriptive statistics for health outcomes", message=FALSE, warning=FALSE}
...
healthdf.table<- healthdf %>%
  ...
  summarize(
    ...
    min_health_outcome_rank = min_rank(mean_Health_Outcome_rank),
...
Table \@ref(tab:Health-Outcomes-Table): Descriptive statistics for health outcomes

**Community Health Z Score Summary Table**
```{r Health-Z-Score-Summary-Table, echo=TRUE, message=FALSE, warning=FALSE}
...
Table \@ref(tab:Health-Z-Score-Summary-Table): Health Indicators Z Score Summary Table
...
```{r ACS-Summary-Table, echo=TRUE, fig.align='left', fig.cap="Summary table of county-level explanatory socioeconomic variables in Pennsylvania. Data from ACS 1-year estimates.", message=FALSE, warning=FALSE}

pa.table<-pa.county.summary %>%
  select(Variable=rowname, Observations=n, Median=median, Mean=mean, SD=sd, Range=range, Minimum=min, Maximum=max) 
kable(x=pa.table,
  format = "html",
  caption="ACS Summary Table")%>%
  kable_classic(full_width = F, html_font = "Cambria")

```
Table \@ref(tab:ACS-Summary-Table): Summary table of county-level explanatory socioeconomic variables in Pennsylvania. Data from ACS 1-year estimates.
...
```{r multiple linear regression wrangling, echo=TRUE}
...
```{r time series analysis wrangling, echo=TRUE}
...
```{r Coal-prod-over-time, echo=TRUE, fig.align='left', fig.cap="Coal Production plotted over time at each Pennsylvania County", fig.height=10, fig.width=20, message=FALSE, warning=FALSE}
...
This figure shows that most of the counties with a high Z score, darker orange indicating poor health, are shown on the right panel indicating coal producing counties. There are a few exceptions, for example, the County of Philadelphia, which also includes the City of Philadelphia, the largest city in the State of Pennsylvania (U.S. Census, 2020). The other major exception is the County of Centre, the county shown middle, which had relatively low total coal production (2,543,689 tons) compared to neighboring counties.
...
```
Map C: Map of Z scores Health Outcome by County in 2010
...
```
Map D: Map of Health outcomes ranking by county in 2010
...
**Plot of socioeconomic variables for all PA counties**
```{r dem-counties-graph, echo=TRUE, fig.align='left', fig.cap="Graph depicting median age, median income, total population, percent of population with a bachelor’s degree and percent of population which is white-identifying for Pennsylvania counties in 2019. Data sourced from ACS 1-year estimates.", fig.height=10, fig.width=20}
...
```{r time-z-all-counties, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of health outcomes and health factors z scores over time across all counties", fig.height=2.5, message=FALSE, warning=FALSE}
...
```{r sum-coal-over-time, echo=TRUE, fig.align='left', fig.cap="Plot of coal production over time across all counties", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Linear regression of total coal production for each year by county health factor**
```{r code for linear regression time series analysis of all counties factor, echo=TRUE, message=FALSE, warning=FALSE}
...
**Linear regression of total coal production for each year by county health outcome**
```{r code for linear regression time series analysis of all counties outcome, echo=TRUE, message=FALSE, warning=FALSE}
...
**Schuykill County**
```{r schuylkill-health-factors, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of coal production and health factors z score over time in the Schuylkill county", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Regression of coal production and health factors z score over time in the Schuylkill county**
...
```{r schuylkill-health-outcomes, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of coal production and health outcomes z score over time in the Schuylkill county", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Regression of coal production and health outcomes z score over time in the Schuylkill county**
...
**Armstrong County**
```{r armstrong-health-factors, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of coal production and health factors z score over time in the Armstrong county", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Regression of coal production and health factors z score over time in the Armstrong county**
...
```{r armstrong-health-outcomes, echo=FALSE, fig.align='left', fig.cap="Synchronous plot of coal production and health outcomes z score over time in the Armstrong county", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Regression of coal production and health outcomes z score over time in the Armstrong county**
...
**Clearfield County**
```{r clearfield-health-factors, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of coal production and health factors z score over time in the Clearfield county", fig.height=2.5, message=FALSE, warning=FALSE}
...
**Regression of coal production and health factors z score over time in the Clearfield county**
...
```{r clearfield-health-outcomes, echo=TRUE, fig.align='left', fig.cap="Synchronous plot of coal production and health outcomes z score over time in the Clearfield county", fig.height=2.5, message=FALSE, warning=FALSE}
...
doubleYScale(obj1, obj2, add.ylab2 = TRUE, use.style=FALSE)
```
**Regression of coal production and health outcomes z score over time in the Clearfield county**
...
acs.health.coal.cor.edit <- data.edit %>% 
  ...
  rename(temporal.distance=year.from.peak.2019) %>% 
  na.omit()

#Run correlation plot
acs.health.coal.Corr <- cor(acs.health.coal.cor.edit)
mixed.corr<-corrplot.mixed(acs.health.coal.Corr, upper = "circle")
```
...
```{r Linear regression model 1 results, echo=TRUE, fig.align='left', fig.cap="Initial regression results including all socioeconomic and coal production variables regressed on health outcomes z-score. Adj. R-squared = 0.5189, message=FALSE, warning=FALSE, 14 degrees of freedom.", fig.height=2.5, Echo=FALSE}
...
**AIC Results**
```{r AIC results, echo=TRUE, fig.align='left', fig.cap="Akaike Information Criteria (AIC) stepwise results", fig.height=2.5, message=FALSE, warning=FALSE, Echo=FALSE}
...
```{r Linear regression model 2 results, echo=TRUE, fig.align='left', fig.cap="Second linear regression following AIC value recommendations for reduced overfitting. Adjusted R-squared: 0.7322, message=FALSE, warning=FALSE, 33 degrees of freedom", fig.height=2.5, Echo=FALSE}
...
**Plots of relationships between explanatory and response variables**
```{r mlr-scatter, echo=TRUE, fig.align='left', fig.cap="Plots to examine linearity assumption of multiple linear regression model.", fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
...
```{r sulfate-plot, echo=TRUE, fig.cap="SO4 Levels Seen in Acid Mine Drainage in Pennsylvania", fig.height=7, message=FALSE, warning=FALSE}
Only.Sulfate <- filter(WaterQualitySlim, Contaminant == "Sulfate as SO4")
...
## References
United States Census Bureau. B01001 SEX BY AGE, 2020 American Community Survey 5-Year Estimates. U.S. Census Bureau, American Community Survey Office. http://www.census.gov/.
...
PA mining history. Department of Environmental Protection. (n.d.). Retrieved December 14, 2022, from https://www.dep.pa.gov/Business/Land/Mining/Pages/PA-Mining-History.aspx 
